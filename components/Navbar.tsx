  ...
  return (
    <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${isScrolled ? 'bg-white/80 backdrop-blur-md border-b border-stone-200 py-4' : 'bg-transparent py-6'}`}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
        <Link href="/" className={`text-2xl font-display font-bold tracking-tighter transition-colors ${isScrolled ? 'text-brand-olive' : 'text-white'}`}>
          {SITE_CONFIG.organization.brandName}
        </Link>
        
        <div className={`hidden md:flex space-x-8 text-sm font-medium uppercase tracking-widest transition-colors ${isScrolled ? 'text-stone-600' : 'text-white/80'}`}>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="relative hidden sm:block">
            <button 
              ...
              className={`flex items-center gap-2 px-3 py-1.5 rounded-full text-xs font-bold transition-all border ${isScrolled ? 'border-stone-200 text-stone-600 hover:bg-stone-50' : 'border-white/20 text-white/80 hover:bg-white/10'}`}
            >
              <Globe size={14} /> {currency}
            </button>
            
            {isCurrencyMenuOpen && (
              <div className="absolute top-full right-0 mt-2 w-24 bg-white rounded-xl shadow-xl border border-stone-100 py-2 overflow-hidden">
                {currencies.map(c => (
                  <button
                    key={c}
                    onClick={() => {
                    ...
                    className={`w-full px-4 py-2 text-left text-xs font-bold hover:bg-stone-50 transition-colors ${currency === c ? 'text-orange-500 bg-orange-50' : 'text-stone-600'}`}
                  >
                    {c}
                  </button>
                ))}
          ...
          <Link href={SITE_CONFIG.whatsapp.waLink} target="_blank" className={`hidden md:block px-6 py-2 rounded-full text-sm font-medium transition-all ${isScrolled ? 'bg-brand-olive text-white hover:bg-brand-olive/90' : 'bg-white text-brand-olive hover:bg-stone-100'}`}>
            Book Now
          </Link>
          <button 
